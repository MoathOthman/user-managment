...
class UserManagementModule extends \yii\base\Module
{
	 ...
	 */
         public $passwordConfig = array(
                    ...
                    'failedAttemptsLimit'=>6,
                    'maxAttempts'=>5,
                    'maxAttemptFailedExpiry'=>60,
                    );
         //public $notUsedExpiry = 30;
         public $notUsedExpiryDay = 30;
	...
	public function init()
	{
                ...
                {
                	$symbolReg = ($this->passwordConfig['symbolContain'])?"(?=.*[$@$!%*?&])":'';
                   // $this->passwordRegexp = "/^(?=.*\d)(?=.*[A-Za-z])".$symbolReg."[$numbContain$ucase$lcase$symContain]"."{"."$minPass,$maxPass"."}"."$/";
                   //symbol allowed
                   $symContain = '!@#$%';
                    $this->passwordRegexp = "/^(?=.*\d)(?=.*[A-Za-z])".$symbolReg."[$numbContain$ucase$lcase$symContain]"."{"."$minPass,$maxPass"."}"."$/";
                    ...
                    if(count($passReqTitle)>1)
                        $this->passwordRequiryTitle = 'Password should be contain '.implode(',', $passtitle).' and '.$passReqTitle[count($passReqTitle)-1]." with minimum ".$minPass.' letters';
                    else
                        $this->passwordRequiryTitle = 'Password should be contain '.implode(',', $passReqTitle)." with minimum ".$minPass.' letters'; 
                }  
