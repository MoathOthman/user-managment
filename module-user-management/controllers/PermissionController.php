...
class PermissionController extends AdminDefaultController
{
	...
	{
                //print_r(Yii::$app->db->driverName);exit;
		$item = $this->findModel($id);
                $auth_item_tb =Yii::$app->getModule('user-management')->auth_item_table;
                $auth_item_detail_tb = Yii::$app->getModule('user-management')->auth_item_detailed_table;
		 ...
		 //Route::find()->asArray()->all();
                    if(Yii::$app->db->driverName == 'mysql')
                    {
                    $routes = Route::find()
                    ->leftJoin($auth_item_detail_tb, $auth_item_detail_tb.'.`name` = '.Yii::$app->getModule('user-management')->auth_item_table.'.`name`')
                    ->select([$auth_item_tb.'.name',$auth_item_tb.'.type',$auth_item_detail_tb.'.description', $auth_item_tb.'.rule_name',$auth_item_tb.'.data',$auth_item_tb.'.created_at',$auth_item_tb.'.updated_at',$auth_item_tb.'.group_code',])
                    ->asArray()
                    ->all();
                    }
                    else 
                    {
                     $routes = Route::find()
                    ->select(['auth_item.name','auth_item.type','auth_item_detailed.description', 'auth_item.rule_name','auth_item.data','auth_item.created_at','auth_item.updated_at','auth_item.group_code',])
                    ->leftJoin('auth_item_detailed', '[auth_item_detailed].[name] = [auth_item].[name]')
                    
                    ->asArray()
                    ->all();
                    }
            
                $routes_alias = AuthItemDetailed::find()->asArray()->all();
...
//                    $routes[]
		...
		$childPermissions = AuthHelper::getChildrenByType($item->name, AbstractItem::TYPE_PERMISSION);
                ...
                //$childRoutes = AuthHelper::getChildrenByType(!empty($item->description)?$item->description:$item->name, AbstractItem::TYPE_ROUTE);
		//$childPermissions = AuthHelper::getChildrenByType(!empty($item->description)?$item->description:$item->name, AbstractItem::TYPE_PERMISSION);
