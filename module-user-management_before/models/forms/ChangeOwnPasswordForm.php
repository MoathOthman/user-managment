	...
	public function rules()
	{
		...
		$return = [
			...
			['current_password', 'validateCurrentPassword', 'except'=>'restoreViaEmail'],
                ...
                if(!empty($filterText))
                {
                   $return[]= ['password', 'match', 'pattern' => '/^((?!'.$filterText.').)*$/i', 'message' => 'Your password could not contain your username fff'.$filterText];
                }
                return $return;
	...
	public function changePassword($performValidation = true)
	{
		if ( $performValidation AND !$this->validate() )
		{
			return false;
                ...
                if (preg_match('/^(.)*'.$this->user->username.'(.)*$/i', $this->password)  )
		{
