	...
	public function up()
	{
		...
		if ($this->db->driverName === 'mysql') {
			...
			$this->createTable($auth_item_table_name, [
				...
				'updated_at' => Schema::TYPE_INTEGER,
				'PRIMARY KEY (name)',
				'FOREIGN KEY (rule_name) REFERENCES ' . Yii::$app->getModule('user-management')->auth_rule_table . ' ',
			], $tableOptions);
			...
			$this->createTable($auth_item_child_table_name, [
				...
				'FOREIGN KEY (parent) REFERENCES ' . Yii::$app->getModule('user-management')->auth_item_table . ' ',
				'FOREIGN KEY (child) REFERENCES ' . Yii::$app->getModule('user-management')->auth_item_table . ' ',
			], $tableOptions);
	        }
