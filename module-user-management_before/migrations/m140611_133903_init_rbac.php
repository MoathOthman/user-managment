	...
	public function up()
	{
		...
		if ($this->db->driverName === 'mysql') {
			$tableOptions = 'CHARACTER SET utf8 COLLATE utf8_general_ci ENGINE=InnoDB';
                        $ref_integrity = ' (name) ON DELETE SET NULL ON UPDATE CASCADE';
                        $ref_integrity2 = ' (name) ON DELETE CASCADE ON UPDATE CASCADE';
                        $ref_integrity3 = ' (id) ON DELETE CASCADE ON UPDATE CASCADE';
		}
			...
			$this->createTable($auth_item_table_name, [
				...
				'updated_at' => Schema::TYPE_INTEGER,
				'PRIMARY KEY (name)',
				'FOREIGN KEY (rule_name) REFERENCES ' . Yii::$app->getModule('user-management')->auth_rule_table . $ref_integrity,
			], $tableOptions);
			...
			$this->createTable($auth_item_child_table_name, [
				...
				'FOREIGN KEY (parent) REFERENCES ' . Yii::$app->getModule('user-management')->auth_item_table . $ref_integrity2,
				'FOREIGN KEY (child) REFERENCES ' . Yii::$app->getModule('user-management')->auth_item_table . $ref_integrity2,
			], $tableOptions);
	        }
			...
			$this->createTable($auth_assignment_table_name, [
				...
				'FOREIGN KEY (item_name) REFERENCES ' . Yii::$app->getModule('user-management')->auth_item_table . $ref_integrity2,
				'FOREIGN KEY (user_id) REFERENCES '.Yii::$app->getModule('user-management')->user_table.$ref_integrity3,
			], $tableOptions);
	        }
	}
